...
template<class T> struct Seg { 
	const T ID = -1; // comb(ID,b) must equal b
...
int main() {
	...
	F0R(i, M) {
		int l, r; cin >> l >> r; --l; --r;
		ranges[r].push_back({l, r});
	}

	Seg<int> seg; seg.init(MAXN);
	vector<vector<pair<pii, int>>> data (N);
	F0R(i, N) {
		seg.update(arr[i], i);

		for (pii range : ranges[i]) {
			int val = seg.getMex(range.first);

			data[range.first].push_back({range, val});
		}
	}
	...
	R0F(i, N) {
		dp[i] = dp[i+1];
		for (auto r : data[i]) {
			dp[i] = max(dp[i], dp[r.first.second + 1] + r.second);
		}
	}
	print(dp[0]);
}